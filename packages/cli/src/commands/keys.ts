...
export const keysCommand = new Command("keys")
  ...
  .addCommand(
    new Command("create")
      ...
      .action(async (name, options) => {
        console.log(chalk.yellow("\n⚠️  cortera keys create is not yet implemented"));
        ...
        console.log(chalk.gray(`   cortera keys create "${name}" --actor-id ${options.actorId || "<actor-id>"} --actor-type ${options.actorType}`));
        process.exit(0);
      })
  )
  .addCommand(
    new Command("list")
      ...
      .action(() => {
        console.log(chalk.yellow("\n⚠️  cortera keys list is not yet implemented"));
  ...
  .addCommand(
    new Command("revoke")
      ...
      .action(() => {
        console.log(chalk.yellow("\n⚠️  cortera keys revoke is not yet implemented"));
        console.log(chalk.gray("   This command depends on Stage 9 (auth/API key management).\n"));
        process.exit(0);
      })
  );